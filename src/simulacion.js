...
class LlegadaCliente {
  ...
  ocurreEvento(datos) {
    ...
    if (datos.stock === 0) {
      ...
      datos.colaEventos.push(
        new ClienteSeRetiraPorTiempo(this.tiempo, this.nroCliente)
      );
    }
  }
}
...
class FinAtencion {
  ...
  ocurreEvento(datos) {
    ...
    if (datos.stock === 0) {
      //CAMBIAR TODOS LOS QUE ESTAN EN COLA (ESPERANDO ATENCION) PASAN A SER CLIENETS TRISTES
      datos.clientesTristes += 1;
    } else {
      datos.stock =
        productosPedidos > datos.stock ? 0 : datos.stock - productosPedidos;

      if (datos.stock === 0) {
        datos.clientesTristes += datos.colaClientes.length;
        datos.colaClientes = [];
        datos.colaEventos = datos.colaEventos.filter(
          (evento) => !(evento instanceof EncendidoHorno45Minutos)
        );
    ...
    if (datos.colaClientes.length > 0) {
      ...
      datos.colaEventos = datos.colaEventos.filter(
        (evento) =>
          !(
            evento instanceof ClienteSeRetiraPorTiempo &&
            evento.nroCliente === nroCliente
          )
      );
    } else {
      datos.empleadosLibres += 1;
...
class FinCoccionHorno {
  constructor(tiempoActual, stock) {
    //TODOS LOS QUE ESTAN EN COLA (ESPERANDO ATENCION) PASAN A SER CLIENETS TRISTES
    if (stock === 0) {
      this.produccion = 45;
...
class ClienteSeRetiraPorTiempo {
  ...
  ocurreEvento(datos) {
    datos.colaClientes = datos.colaClientes.filter(
      (cliente) => cliente !== this.nroCliente
    );
  }
}
